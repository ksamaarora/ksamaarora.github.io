...
At Microsoft, AI software development is guided by **6 principles**: **Fairness**, **Reliability and Safety**, **Privacy and Security**, **Inclusivness** (includes all - everyone benefits regardless of physical ability, gender, ethnicity etc), **Transparency**, **Accountability** (developers should work within principles that meet legal standards)
(FRSPS ITA)
...
**Azure Automated ML:** automatically trains ML model by providing dataset and picking label i.e. **automatically pick algo**.

**Azure Designer:** provides **drag-and-drop interface/ canvas** to train, test and deploy ML models - enables to **save progress** as a **pipeline draft** - ~~doesnt includes custom JS functions~~.
...
**- AUC** is a **numerical measure** of the **ROC curve's performance** - area under RUC - closer to 1 better - TPR v/s FPR - random guessing is 0.5 (0.5-0.7 poor, 0.7-0.8 ok, 0.8-0.9 very good)
...
- **Azure Computer Vision Read API:** provides quick extraction of small amount of text in image - interact with the service via API endpoint and key
...
Custom vision Service - Classify images, detect obejcts 
...
**Azure AI Custom Vision:** image recognition service to build, deploy and improve **your own image identifier model** - **you provide your own data to train model** - can specify **whether used for training, prediction or both**(Cognitive service uses both)
...
- **Text Analytics:** includes **language detection**, **sentiment analysis** - if text +ve (1) or -ve (0), **extract key phrases** (to summarize doc or find key talking points), **entity recognition** (return list of entities, categories {like people, dates, times, URLs}, confidence score)
- **Speech Detection**
...
- **Feature Selection:** Process of selecting the key subset of features - eliminate irrelevant, redundant features
