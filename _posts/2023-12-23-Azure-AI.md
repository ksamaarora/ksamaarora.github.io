...
- **Tokenization:** First step is to analyze corpus and break it down into tokens 
  - Following concepts may be applied based on NLP problem:
    ...
    - **Stop word removal:** stop words are those words that should be excluded from the analysis - e.g. "the", "a" or "it" make easier for people to read but exclusing them will make it better to identify imp words
