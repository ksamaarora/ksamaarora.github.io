...
- **Classification** - used to predict a category or class 
  ...
  - **Multiclass classification** - predicts multiple possible classes - may be more than one valid label for single obs - e.g. movie can be both scifi and comedy - 2 types of algo can be used - 
...
Note: When yu write code, keys and endpoints must be included in authentication header, which sends authorization key to service to confirm that the app can use resource. 
...
**Convolutional Neural Network (CNN)*** - common ML model architecture for computer vision - uses filters to extract numeric feature maps, and then feed feature values into deep learning model to generate label prediction - 
...
**NLP Services: (TSTLA) IMP**

- **Text  (Azure AI Language):** includes: 
  ...
  - **Entity linking** - identifies **known entitites** together, **link to Wikipedia**
...
- **Speech recognition** - ability to detect and interpret spoken input - convert **speech into text** (Speech Text) - spoken words can be **recorded voice, audio file, live audio** - uses models 
  - **Acoustic model**: converts audio signals into phonemes (representations of specific sounds)
...
- **Azure AI Language (now Azure Cognitive Service for Language):** **used to create knowledge database of qs ans** - **enter manually, or import FAQ, web page or chit-chat data** - then publish so that Azure bot service can deliver to end users - can be interacted **via REST API, software development kit, and Language studio**
