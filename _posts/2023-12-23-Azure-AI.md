...
**4) Deploy a predictive service:** 
	After creating the interference pipeline, you can **deploy it as an endpoint**. Deployment state can be viewed on the Details page and will indicated **Healthy when deployment is complete**. On Test tab, you can test your deployed service with a sample data in **JSON format** (used to **indicate recording pattern of data**) You can find credentials for your service on Consume tab. 
...
**Azure AI Custom Vision:** image recognition service to build, deploy and improve **your own image identifier model** - **you provide your own data to train model** - can specify **whether used for training, prediction or both**

- **Cognitive service:** can be used **for both training and prediction** - uses only **one key and endpoint**
...
- **Feature Selection:** Process of selecting the key subset of features - eliminate irrelevant, redundant features (~~not help in splitting data~~)
